...
let item = { name: "bicycle", price: 300 }
...
describe("POST /items", async function(){
    test("Creates a new shopping list item", async function(){
        ...
        .send({ name: "umbrella", price: 50 });
        expect(response.statusCode).toBe(200);
        expect(response.body.item).toHaveProperty("name");
        expect(response.body.item).toHaveProperty("price");
        expect(response.body.item.name).toEqual("umbrella");
        expect(response.body.item.price).toEqual(50); 
    });
});
...
describe("PATCH /items/:name", async function(){
    test("Updates a single item", async function(){
      const response = await request(app)
        ...
        .send({ name: "PS5" });
      expect(response.statusCode).toBe(200);
      expect(response.body.item).toEqual({ name: "PS5" });
    });
